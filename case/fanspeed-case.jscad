function main() {
  ...
  var box = cube().scale([w + 2 * thick, h + 2 * thick, depth + 2 * thick]).translate([-1, -1, -1])
    ...
    // encoder axis hole
    .subtract(cylinder({r: encoderAxisDiam / 2, h: 10}).rotateX(-90).translate([encoderAxisX, -thick, 23.8]))
    // dc jack hole
    ...
    var upperPartBounds = cube().translate([-0.5, -0.5, 0]).scale(100).translate([0, 0, partCutY]);
    var lowerPart = box.subtract(upperPartBounds);
    var upperPart = box.intersect(upperPartBounds);
    return union(lowerPart,
      upperPart.translate([0, 4, -depth]).rotateX(180)).translate([0, 0, 1]);
}
